// testComplexQueryPerformance.js
const { Client } = require('pg');
const { performance } = require('perf_hooks');

// Create a new PostgreSQL client instance
async function testComplexQueryPerformance() {
    const client = new Client({
        connectionString: 'postgresql://postgres:1234@localhost:5432/postgres'
    });
    client.connect()
      .then(() => console.log('Connected to PostgreSQL database'))
      .catch(err => console.error('Error connecting to PostgreSQL database', err));
  // Adjust the query according to your specific complex query
  const query = 'SELECT * FROM users WHERE age > 50';
  const start = performance.now();
  try {
    await client.query(query);
    const end = performance.now();
    const duration = end - start;
    console.log(`Complex query operation duration: ${duration} ms`);
  } catch (err) {
    console.error('Error performing complex query operation in PostgreSQL', err);
  } finally {
    client.end()
      .then(() => console.log('Closed PostgreSQL database connection'))
      .catch(err => console.error('Error closing PostgreSQL database connection', err));
  }
}

module.exports = testComplexQueryPerformance;